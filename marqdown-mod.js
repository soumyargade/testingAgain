...
function EscapeCode(text) {
    return text.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/[*]/g, '\\*').replace(/_/g, '\\_').replace(/[{][}]/g, '\\{}').replace(/\[\]/g, '\\[]').replace(/[(][)]/g, '\\()');
}
function ProcessTokens(tokens, qOptions) {
    ...
    newTokens.push({
        type: 'text',
        text: pageHeader.format(pageNumber++)
    });
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (t.type == 'heading' && t.depth == 3) {
            if (questionNumber > 0) {
                ...
                if (lastToken.type == 'text' && lastToken.text.indexOf('data-page=') >= 0) {
                    newTokens.splice(newTokens.length - 3, 0, {
                        type: 'text',
                        text: '</div>'
                    });
                } else {
                    newTokens.splice(newTokens.length - 1, 0, {
                        type: 'text',
                        text: '</div>'
                    });
                }
        ...
        if (t.type == 'table') {
            ...
            if (lastRow > 0 && t.cells[lastRow].length != t.cells[lastRow - 1].length) {
                t.cells[lastRow].push('<div>Bug</div>');
            }
            ...
            if (_.any(testChoices, function (col) {
                ...
                html = templates.multichoicetable({
                ...
                html = templates.singlechoicetable({
                    table: questionNumber,
                    headers: headers,
                    rows: bodyRows
                });
...
function processInnerText(innerTokens, tokens) {
    var embeddedHtml = '<div>Bug</div>';
    if (_.all(innerTokens, function (tok) {
            return tok.type == 'text' || tok.type == 'space';
        })) {
        ...
        var filtered = innerTokens.filter(function (tok) {
            return tok.type == 'space' || tok.type == 'text';
        });
