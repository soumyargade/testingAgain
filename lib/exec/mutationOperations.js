    ...
    static rewrite( fileGlob, newPath, operation ) {
        ...
        glob(fileGlob, (err, files) => {
            try {
                ...
                console.log(chalk.green(`File ${filepath} will be mutated results placed into file ${newPath}`));
                ...
                if (operation != "random") {
                    for (let i = 0; i < MutationOperations.operations.length; i++) {
                        if (MutationOperations.operations[i] == operation) op = MutationOperations.operations[i]; 
                    }
                }
